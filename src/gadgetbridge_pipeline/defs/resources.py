...
class FileS3IOManager(dg.ConfigurableIOManager):
    ...
    The S3 key is derived from the asset key plus `extension`, so an asset
    named ["gadgetbridge", "raw", "foo"] round-trips through
    "gadgetbridge/raw/foo<extension>".
    """
    bucket: str
    endpoint_url: str
    extension: str
    ...
    def _key(self, asset_key):
        return '/'.join(asset_key.path) + self.extension
